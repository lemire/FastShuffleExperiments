...
template <randfnc32 rfnc32> void ShuffleBenchmark32(size_t size, bool verbose, bool prefetch) {
  ...
  AVG_TIME_NS(shuffle_go32<rfnc32>(testvalues, size),
  ...
  AVG_TIME_NS(shuffle_java32<rfnc32>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;

  AVG_TIME_NS(shuffle_nearlydivisionless32<rfnc32>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;

  AVG_TIME_NS(shuffle_floatmult32<rfnc32>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
  AVG_TIME_NS(shuffle_precomputed(testvalues, size, precomputed),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
  ...
  AVG_TIME_NS(std::shuffle(testvalues, testvalues + size, gen),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
#endif
  printf("     ");
  AVG_TIME_NS(buf_shuffle_go32<rfnc32>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;

  AVG_TIME_NS(buf_shuffle_java32<rfnc32>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;

  AVG_TIME_NS(buf_shuffle_nearlydivisionless32<rfnc32>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;

  AVG_TIME_NS(buf_shuffle_floatmult32<rfnc32>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
...
template <randfnc64 rfnc64> void ShuffleBenchmark64(size_t size, bool verbose, bool prefetch) {
  ...
  AVG_TIME_NS(shuffle_go64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;

  AVG_TIME_NS(shuffle_java64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
  AVG_TIME_NS(shuffle_nearlydivisionless64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
  AVG_TIME_NS(shuffle_floatmult64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
  AVG_TIME_NS(shuffle_precomputed(testvalues, size, precomputed),
  ...
  AVG_TIME_NS(buf_shuffle_go64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;

  AVG_TIME_NS(buf_shuffle_java64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
  AVG_TIME_NS(buf_shuffle_nearlydivisionless64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
  AVG_TIME_NS(buf_shuffle_floatmult64<rfnc64>(testvalues, size),
               prefetch ? array_cache_prefetch(testvalues, size) : array_cache_flush(testvalues, size), repeat, size, verbose);
  if (sortandcompare && (sortAndCompare(testvalues, pristinecopy, size) != 0))
    return;
